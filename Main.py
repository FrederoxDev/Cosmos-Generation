...
#     if y == ground_height + 1 and feature > 0 and len(biome["features"]) > 0:
#         if feature_2 > 0.4:
...
#     elif y == ground_height + 1 and flora > 0 and len(biome["flora"]) > 0:
...
def Get2DNoiseArr(arr_width, offset, scale):
    arr = np.empty((arr_width, arr_width))

    for x in range(arr_width):
        for z in range(arr_width):
            arr[x, z] = Get2DNoise(x + planet_offset, z + planet_offset, offset, scale)

    return arr

def Get2DNoiseArrThreaded(arr_width, offset, scale, results, index):
    results[index] = Get2DNoiseArr(arr_width, offset, scale)

if __name__ == "__main__":
    start = time.time()
    results = [None] * 5

    t1 = threading.Thread(target=Get2DNoiseArrThreaded, args=(width * 16, 3124, 0.2, results, 0))
    t2 = threading.Thread(target=Get2DNoiseArrThreaded, args=(width * 16, 43562, 0.2, results, 0))
    # t3 = threading.Thread(target=Get2DNoiseArrThreaded, args=(width * 16, 4314, 1, results, 0))
    # t4 = threading.Thread(target=Get2DNoiseArrThreaded, args=(width * 16, 65432, 10, results, 0))
    # t5 = threading.Thread(target=Get2DNoiseArrThreaded, args=(width * 16, 252, 2, results, 0))
    ...
    print("Noise Maps Pool: " + str(time.time() - start) + "s")

    # Generate Noise Values
    start = time.time()

    temperature_map = Get2DNoiseArr(width * 16, 3124, 0.2)
    rainfall_map = Get2DNoiseArr(width * 16, 43562, 0.2)
    # flora_map = Get2DNoiseArr(width * 16, 4314, 1)
    # feature_threshold_map = Get2DNoiseArr(width * 16, 65432, 10)
    # feature_placement_map = Get2DNoiseArr(width * 16, 252, 2)

    print("Noise Maps: " + str(time.time() - start) + "s")
